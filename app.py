...
# Initialize MediaPipe outside the loop for web optimization
@st.cache_resource
...
class VideoProcessor(VideoTransformerBase):
    ...
    def transform(self, frame):
        ...
        # Web Background Calibration (takes first 30 frames)
        if self.calibrate_frames < 30:
            self.background = img.copy()
            self.calibrate_frames += 1
            cv2.putText(img, f"CALIBRATING BACKGROUND... {int((self.calibrate_frames/30)*100)}%", (20, 40),
        ...
        if len(pinch_positions) == 2:
            ...
            if (x2 - x1) > 15 and (y2 - y1) > 15:
                ...
                full_invisible = np.where(condition, self.background, img)
...
# Start WebRTC Video Streamer with public STUN servers for routing connectivity
webrtc_streamer(
